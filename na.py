...
# ==========================================
# DYNAMIC DATABASE LOADER FUNCTION (Gaaffii Sadii: Salphaa, Giddu-galeessaa, Cimaa)
# ==========================================
python_difficulty_map = {
    "Salphaa": ["salphaa", "easy", "low", "1"],
    "Giddu-galeessaa": ["giddu-galeessaa", "medium", "moderate", "2"],
    "Cimaa": ["cimaa", "hard", "high", "3"]
}
    ...
    def select_3_levels(pool):
        ...
        for level_name in ["Salphaa", "Giddu-galeessaa", "Cimaa"]:
            ...
            if sub_pool:
                ...
                # Fallback yoo leveliin hin barreeffamne ta'e pool irraa fudhachuuf
                if pool:
                    rem_pool = [p for p in pool if p not in selected]
                    if rem_pool:
                        selected.append(random.choice(rem_pool))
        # Yoo haga 1 ykn 2 ta'an guutuuf
