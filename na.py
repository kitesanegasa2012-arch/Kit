...
# IN-MEMORY MASTER QUESTION BANK (KUUSAA GAAFFII BARSIISAA DIREETTI FAYYADAMU)
...
def load_databases_for_grade(grade_str):
    # Dheerina/Faayilii JSON alaa malee Kuusaa Barsiisaa Direct fayyadama
    ...
    return {
        "afaan_oromoo": select_6_questions(bank["afaan_oromoo"]),
        "math": select_6_questions(bank["math"]),
        "english": select_6_questions(bank["english"])
    }
    ...
    with col1:
        if str_app.button("Gara Appiitti Darbi"):
            if name.strip():
                ...
                # Direct Kuusaa Gaaffii Barsiisaa irraa fudhata
...
def afaan_oromoo_screen():
    ...
    if not questions:
        str_app.warning("Gaaffiin Afaan Oromoo faayilii kana keessatti hin argamne.")
...
def math_screen():
    ...
    if not questions:
        str_app.warning("Gaaffiin Herregaa faayilii kana keessatti hin argamne.")
        if str_app.button("🏠 Gara Manayeessaa"):
            str_app.session_state.current_page = "home"
            str_app.rerun()
        return
